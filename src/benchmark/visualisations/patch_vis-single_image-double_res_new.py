...
from src.benchmark.pretrain_ssl.datasets.SSL4EO.ssl4eo_dataset_lmdb_new import load_meta_df, LMDBDatasetS2C
...
def get_brightness_factor(img_rgb):
    if img_rgb.max() < 80:
        return 2
    else:
        return 1.5
...
out_folder = '/gpfs/work5/0/prjs0790/data/visualisations/'
...
# Prepare data outputting
cluster_folder = out_folder + f'n_clu_{n_cluster}/'
prepare_dir(cluster_folder)
...
# img = img_list[5]
    ...
    brightness_factor = get_brightness_factor(img_rgb)
    ...
    # Split the image
    img_splitted = [j for k in np.split(img_to_split, n_image_split, 1) for j in np.split(k, n_image_split, 2)]
    ...
    plt.savefig(cluster_folder + f'original_image_{i}.png')

    plt.imshow(np.transpose(pre_odin_image, (1, 2, 0)))
    plt.tight_layout()
    plt.title(f'Pre odin clustering {i}')
    plt.savefig(cluster_folder + f'pre_odin_{i}.png')

    plt.imshow(np.transpose(post_odin_image, (1, 2, 0)))
    plt.tight_layout()
    plt.title(f'Post odin clustering {i}')
    plt.savefig(cluster_folder + f'post_odin_{i}.png')

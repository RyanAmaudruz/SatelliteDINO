...
for ji, img1 in enumerate(img_list[:20]):
    for ij in range(4):

        # Sample the single image
        img1rgb = np.flip(img1[ij, 1:4, :, :], 1)

        # Add more colour to improve visibility
        ...
        first_split = np.split(img1[ij], 2, 1)
        ...
        data_dic[f'img_{ji}_season_{ij}'] = pred_list1

new_dic = {}
for k, v in data_dic.items():
    new_dic[k] = []
    for p1 in pred_list1:
        new_dic[k].append(torch.concat([p[:, 1:, :].reshape(14, 14, 384) for p in p1], 0))
...
mask = torch.concat(all_emb, 1).to('cpu')

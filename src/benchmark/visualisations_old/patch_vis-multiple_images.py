...
img_rgb_list = [np.flip(img[0, 1:4, :, :], 1) for img in img_list]
img_rgb_list2 = [(img * 2).clip(0, 256).astype(int) for img in img_rgb_list]
...
for i in range(4):
    ...
    img1rgb = np.flip(img1[i, 1:4, :, :], 1)

    img1rgb2_list.append((img1rgb * 2).clip(0, 256).astype(int))
...
pred_list = []
for img_plus_sea in img_list:
    for sea in range(4):
        arr_t = val_transform(np.transpose(img_plus_sea[sea], (1, 2, 0)))
        pred = model(arr_t[None, :, :, :].float().to('cuda'))
        pred_reshaped = pred[:, 1:, :].reshape(14, 14, 384).to('cpu').detach()
        pred_list.append(pred_reshaped)

all_emb = np.concatenate([p.reshape(-1, 384) for p in pred_list])
...
cluster_pred = kmeans.fit_predict(all_emb)

cluster_pred_torch = torch.from_numpy(cluster_pred)

for count in range(4):
    extra_count = 14*14 * count

    start_index = 0 + 14*14 * 6 * 4 + extra_count
    end_index = start_index + 14*14

    rel_pred = cluster_pred_torch[start_index: end_index]


    # new_mask = cluster_pred_torch.reshape(56, 56)
    new_mask = rel_pred.reshape(14, 14)
    ...
    img1rgb2_first_channel = img1rgb2_list[count][0, :, :].copy()
    img1rgb2_second_channel = img1rgb2_list[count][1, :, :].copy()
    img1rgb2_third_channel = img1rgb2_list[count][2, :, :].copy()
    ...
    img1rgb2_new = img1rgb2_list[count].copy()
    ...
    plt.savefig(f'/gpfs/home2/ramaudruz/SSL4EO-S12/patch_vis/round_10_cluster_vis-cat_season_{count}.png')
...
#
# plt.imshow(np.transpose(img1rgb2_new))
# plt.savefig(f'patch_vis/round_1_cluster_vis-cat_NEW.png')
#
#
#
#

...
new_band_image = torch.zeros(1, 13, 224, 224)

new_band_image[:, 1:4, :, :] = img2_cropped.flip(0)

# Predict the dense rep and reshape
pred = model(new_band_image.float().to('cuda'))
pred_reshaped = pred[:, 1:, :].reshape(14, 14, 384)

# Reshape for kmeans
mask_sh = pred_reshaped.reshape(-1, 384)
...
new_mask = cluster_pred_torch.reshape(14, 14)

resized_masks = F.interpolate(new_mask.float()[None, None, :, :], size=(224, 224), mode='nearest')

df_t = pd.Series(resized_masks.flatten()).value_counts()


img1rgb2_new = img2_cropped.clone()
...
img1rgb2_first_channel = img2_cropped[0, :, :].clone()
img1rgb2_second_channel = img2_cropped[1, :, :].clone()
img1rgb2_third_channel = img2_cropped[2, :, :].clone()


for k, v in dict(zip(df_t.index, df_t.values)).items():

    first_cat = (resized_masks == k).reshape(224, 224)
...
img1rgb2_new = (((img1rgb2_new * 255) + np.concatenate([
...
plt.imshow(np.transpose(img1rgb2_new, (1, 2, 0)))
plt.savefig(f'/gpfs/home2/ramaudruz/SSL4EO-S12/patch_vis/real_image_vis4.png')

...
def load_meta_df():
    meta_df = pd.read_csv('/gpfs/work5/0/prjs0790/data/ssl4eo-s12/s2a_un/s2a_tree_metadata.csv')
...
def load_patch(patch_id, snapshot, band):
    # data_dir = '/gpfs/scratch1/shared/ramaudruz/data3/BigEarthNet-v1.0/'
    # im_arr = imageio.imread(data_dir + '/' + patch_id + f'/{patch_id}_{band}.tif')
    # return im_arr
    file_path = f'/gpfs/scratch1/shared/ramaudruz/ben_full_extract_{band}.h5'
    with h5py.File(file_path, 'r') as f:
        # patch_arr = np.array(f.get(f'{patch_id}/{snapshot}'))
        patch_arr = np.array(f.get(f'{patch_id}'))
    return patch_arr

# for b in ['B01', 'B02', 'B03', 'B04', 'B05', 'B06', 'B07', 'B08', 'B8A', 'B09', 'B11', 'B12']:
#     t = imageio.imread(data_dir + '/' + patch_id + f'/{patch_id}_{b}.tif')
#     # print(t)
#     #
#     print([b, t.shape])
...
# with h5py.File('/gpfs/scratch1/shared/ramaudruz/ben_full_extract_B01.h5', 'r') as f:
#     t = list(f.items())
#
# pd.DataFrame({'t': t}).to_csv('/gpfs/work5/0/prjs0790/data/bigearthnet/ben_full_extract_content.csv', index=False)

ref_df = pd.read_csv('/gpfs/work5/0/prjs0790/data/bigearthnet/ben_full_extract_content.csv')
ref_df['t2'] = ref_df['t'].map(lambda x: x.split("'")[1])


data_dir = '/gpfs/scratch1/shared/ramaudruz/data3/BigEarthNet-v1.0/'

# dir_list = os.listdir(data_dir)
dir_list = ref_df['t2'].to_list()
...
if set_type == 'train':
    train_df = pd.read_csv('/gpfs/work5/0/prjs0790/data/bigearthnet/bigearthnet-train.txt', header=None)
    train_files = set(train_df[0].tolist())
    train_dirs = [d for d in dir_list if d in train_files]
    files_to_process = train_dirs
elif set_type == 'val':
    val_df = pd.read_csv('/gpfs/work5/0/prjs0790/data/bigearthnet/bigearthnet-val.txt', header=None)
    val_files = set(val_df[0].tolist())
    val_dirs = [d for d in dir_list if d in val_files]
    files_to_process = val_dirs

#
# meta_df = load_meta_df()

def new_get_array(files_to_process, patch_id):
    # data_root_patch = os.path.join(self.root, mode, patch_id)
    # patch_seasons = os.listdir(data_root_patch)
    # patch_seasons = files_to_process
    # seasons = []
    ...
    for i, band in enumerate(bands):
        raw_patch = load_patch(patch_id, patch_id_season, band)
        # ch = cv2.resize(raw_patch, dsize=(264, 264), interpolation=cv2.INTER_LINEAR_EXACT)
        ch = cv2.resize(raw_patch, dsize=(128, 128), interpolation=cv2.INTER_CUBIC)
        chs.append(ch)
    img = np.stack(chs, axis=0) # [C,264,264]
    # seasons.append(img)
    # img_4s = np.stack(seasons, axis=0) # [4,C,264,264]
...
# patch_ids = meta_df['patch_id'].unique().tolist()

import time
data_list2 = []
start = time.time()
count = 0
with h5py.File(f'/gpfs/work5/0/prjs0790/data/bigearthnet/t_s2a_128_all_{set_type}.h5', 'w') as f:
    meta_df = None
    for i, p in enumerate(files_to_process):
        new_array = new_get_array(meta_df, p)
        f.create_dataset(str(p), data=new_array)
        if i % 1000 == 0:
            print(i)

...
class FakeArgs:
    ...
    bands = 'B13'
    batch_size = 16
    checkpoints = '/gpfs/work5/0/prjs0790/data/run_outputs/checkpoints/ssl4eo_ssl/moco_s2c_new_transforms_wo_temp'
    crop_min = 0.08
    data = ''
    dist_url = ''
    distributed = False
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        batch_time, data_time, learning_rates, losses = train(train_loader, model, optimizer, scaler, summary_writer, epoch, args)
        log_stats = {'learning_rates': learning_rates.avg, 'train_loss': losses.avg, 'epoch': epoch}
        wandb.log(log_stats)
        if epoch%5==4:
...
def train(train_loader, model, optimizer, scaler, summary_writer, epoch, args):
    ...
    return batch_time, data_time, learning_rates, losses

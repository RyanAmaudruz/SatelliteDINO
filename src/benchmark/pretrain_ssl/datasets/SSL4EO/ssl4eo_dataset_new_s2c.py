...
def load_patch(patch_id, snapshot, band):
    file_path = f'/gpfs/scratch1/shared/ramaudruz/ssl4eo_s2_l1c_full_extract_{band}.h5'
...
# meta_df['patch_id'] = meta_df['t'].map(lambda x: x.split("'")[1]).iloc[0]

# file_path = f'/gpfs/scratch1/shared/ramaudruz/ssl4eo_s2_l1c_full_extract_B1.h5'
#
# data_list = []
#
# for patch_id in meta_df['patch_id']:
#     with h5py.File(file_path, 'r') as f:
#         patch_arr = np.array(f.get(f'{patch_id}'))
#         for i in patch_arr:
#             data_list.append({'patch_id': patch_id, 'timestamp': i})
#
# pd.DataFrame(data_list).to_csv(f'/gpfs/scratch1/shared/ramaudruz/ssl4eo_s2_l1c_full_extract_metadata.csv', index=False)
#
# load_patch(patch_id, patch_id_season, band)


def new_get_array(meta_df, patch_id):
    # data_root_patch = os.path.join(self.root, mode, patch_id)
    # patch_seasons = os.listdir(data_root_patch)
...
# start = time.time()
# count = 0
# with h5py.File('/gpfs/scratch1/shared/ramaudruz/s2c_un/s2c_264_light_new.h5', 'w') as f:
#     for i, p in enumerate(meta_df['patch_id'].unique().tolist()):
...
# data_list = []
#
# with h5py.File('/gpfs/work5/0/prjs0790/data/ssl4eo-s12/s2a_un/s2a_264.h5', 'r') as f:
#     for i in range(1000):
#         data_list.append(np.array(f.get(meta_df['patch_id'].iloc[i])))
#
# c_max = 0
# for i in range(1000):
#     if data_list[i].max() > c_max:
#         c_max = data_list[i].max()

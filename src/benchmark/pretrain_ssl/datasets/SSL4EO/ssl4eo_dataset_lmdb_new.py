...
class LMDBDatasetS2A(Dataset):
    ...
    def __init__(self, lmdb_file, is_slurm_job=False, s1_transform=None, s2a_transform=None, s2c_transform=None, subset=None, normalize=False, mode=['s1','s2a','s2c'], dtype='raw'):
        ...
        # self.meta_df = self.meta_df.iloc[:250880].reset_index(drop=True)
        self.length = self.meta_df['patch_id'].unique().shape[0]
    ...
    def __getitem__(self, index):
        ...
        patch_id = self.meta_df['patch_id'].iloc[index]

        with h5py.File('/gpfs/scratch1/shared/ramaudruz/s2a_uncompressed/s2a_264.h5', 'r') as f:
...
## s2a
        if self.mode==['s2a']:
            # sample_s2a = (data / 10000.0).astype(np.float32)
            sample_s2a = ((data.astype('float32') - np.array(S2A_MEAN).reshape(1, -1, 1, 1)) / np.array(S2A_STD).reshape(1, -1, 1, 1)).astype('float32')
...
class LMDBDatasetS2C(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File('/gpfs/scratch1/shared/ramaudruz/s2c_un/s2c_264_uint16.h5', 'r') as f:
            # data = torch.from_numpy(np.array(f.get(patch_id))).to(torch.float32)
            # data = np.array(f.get(patch_id))
            data = torch.from_numpy(np.array(f.get(patch_id)) / 65535).float()

...
class SegmentationEncoder(torch.nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x1_768 = transforms.Resize(768)(x1)
        x2_768 = transforms.Resize(768)(x2)

        x1_768_out = self.encoder(x1_768)
        x2_768_out = self.encoder(x2_768)

        features += [self.concatenate(
            x1_768_out[:, 1:].permute(0, 2, 1).reshape(-1, 384, 48, 48),
            x2_768_out[:, 1:].permute(0, 2, 1).reshape(-1, 384, 48, 48)
        )]

        x1_384 = transforms.Resize(384)(x1)
        x2_384 = transforms.Resize(384)(x2)

        x1_384_out = self.encoder(x1_384)
        x2_384_out = self.encoder(x2_384)

        features += [self.concatenate(
            x1_384_out[:, 1:].permute(0, 2, 1).reshape(-1, 384, 24, 24),
            x2_384_out[:, 1:].permute(0, 2, 1).reshape(-1, 384, 24, 24)
        )]

        x1_192 = transforms.Resize(192)(x1)
        x2_192 = transforms.Resize(192)(x2)

        x1_192_out = self.encoder(x1_192)
        x2_192_out = self.encoder(x2_192)

        features += [self.concatenate(
            x1_192_out[:, 1:].permute(0, 2, 1).reshape(-1, 384, 12, 12),
            x2_192_out[:, 1:].permute(0, 2, 1).reshape(-1, 384, 12, 12)
        )]

        x1_96 = transforms.Resize(96)(x1)
        x2_96 = transforms.Resize(96)(x2)

        x1_96_out = self.encoder(x1_96)
        x2_96_out = self.encoder(x2_96)

        features += [self.concatenate(
            x1_96_out[:, 1:].permute(0, 2, 1).reshape(-1, 384, 6, 6),
            x2_96_out[:, 1:].permute(0, 2, 1).reshape(-1, 384, 6, 6)
        )]

        x1_48 = transforms.Resize(48)(x1)
        x2_48 = transforms.Resize(48)(x2)

        x1_48_out = self.encoder(x1_48)
        x2_48_out = self.encoder(x2_48)

        features += [self.concatenate(
            x1_48_out[:, 1:].permute(0, 2, 1).reshape(-1, 384, 3, 3),
            x2_48_out[:, 1:].permute(0, 2, 1).reshape(-1, 384, 3, 3)
        )]
